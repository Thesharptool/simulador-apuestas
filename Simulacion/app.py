...
# =========================================================
API_KEY = "9a0c57c7cd90446f9b836247b5cf5c34"  # tu llave que mostraste
NFL_SEASON = "2025REG"  # la que viste que te daba los 32 equipos

def cargar_equipos_nfl():
    """Descarga standings de la NFL y los guarda en session_state."""
...
if ok and data:
    for team in data:
        ...
        key = (team.get("Team") or "").lower().strip()  # DEN, DAL, etc.
        if name:
...
if ok:
    ...
    st.warning("⚠️ No pude traer datos de NFL. Revisa tu suscripción o la llave.")
...
# =========================================================
# 1.a BOTONES PARA RELLENAR DESDE NFL (LOCAL)  ✅ FIX
# =========================================================
if btn_local and ok and local:
    ...
    if team:
        ...
        # guardamos para aplicar después
        st.session_state["to_update_local"] = {
            "l_anota_global": round(anota, 2),
            "l_permite_global": round(permite, 2),
            "local_name": local,
        }
        st.stop()  # paramos aquí para que el siguiente render ya tenga los valores

# aplicamos fuera del clic
if "to_update_local" in st.session_state:
    updates = st.session_state.pop("to_update_local")
    for k, v in updates.items():
        st.session_state[k] = v
    st.experimental_rerun()

# =========================================================
# 1.b BOTONES PARA RELLENAR DESDE NFL (VISITA)  ✅ FIX
# =========================================================
if btn_visita and ok and visita:
    ...
    if team:
        ...
        st.session_state["to_update_visita"] = {
...
if "to_update_visita" in st.session_state:
    updates = st.session_state.pop("to_update_visita")
    for k, v in updates.items():
        st.session_state[k] = v
    st.experimental_rerun()
...
# =========================================================
# 2. PROMEDIOS GLOBAL (los dejamos en blanco por default)
# =========================================================
...
# =========================================================
# 3. PROMEDIOS CASA / VISITA (opcionales)
# =========================================================
...
def proyeccion_suavizada(ofensiva_propia, defensa_rival, es_local=False):
    ...
    if es_local:
        base += 1.5  # pequeño plus por ser local
    return base
...
with c5:
    spread_casa = st.number_input(
        "Spread de la casa (negativo si LOCAL es favorito)", -50.0, 50.0, 0.0, 0.5
    )
with c6:
...
# =========================================================
# 9. DIFERENCIAS vs LÍNEA
# =========================================================
st.subheader("Diferencias vs línea real")

# casa normalmente pone -5.5 si el local es favorito
# nuestro spread_global es (local - visita), así que lo pasamos al mismo formato
...
# =========================================================
# 10. SIMULACIÓN MONTE CARLO (GLOBAL)
# =========================================================
...
# =========================================================
# 11. SIMULACIÓN MONTE CARLO (CASA / VISITA)
# =========================================================
...
# Si hay casa/visita, también los metemos

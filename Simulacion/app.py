...
# ------------------------------------------------
# 4. FUNCIÓN DEL MODELO (la suavizada)
# ------------------------------------------------
def proyeccion_suavizada(ofensiva_propia, defensa_rival, es_local=False):
    # 55% su ataque + 35% lo que permite el rival + 1.5 si es local
...
if hay_cv:
    # si no llenan, usa el global de ese equipo
...
# ------------------------------------------------
# 8. DIFERENCIAS (ya corregido el signo)
# ------------------------------------------------
st.subheader("Diferencias vs línea real")

# nuestro modelo dice “local gana por +X”, pero la casa habla “local -X”
# lo pasamos al idioma del casino:
modelo_spread_formato_casa = -spread_global    # ej. modelo +4.9  -> -4.9
...
# desviación simple: 15% del total o mínimo 5
...
for _ in range(num_sims_global):
    ...
    # para cubrir: (sim_local - sim_visita) + spread_casa >= 0
